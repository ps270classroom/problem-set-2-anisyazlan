...
```
There are 2510 candidates in Challenger, 2022 in Incumbent status, and the other 2482 are holding an Open seat.
...
party_incumbent_promote <- fb_ads |>
  filter(office %in% c("US House", "State Senate")) |> 
  group_by(party, incumbency) |> 
  summarise(promote_prop = mean(ad_tone_promote, na.rm = TRUE),.groups = "drop") |>
  arrange(desc(promote_prop))

party_incumbent_promote

knitr::kable(party_incumbent_promote, col.names = c("Party", "Incumbency", "Average of promotion proportions"), digits = 3)

```

The type of candidate with the highest average is Incumbents from Democrats. 
...
plot_q3 <- ggplot(fb_ads, aes(x = impressions_millions)) + geom_histogram()

plot_q3

```
The histogram looked like it is very left skewed, meaning that the impressions getting are mostly no more than 10 millions impressions on facebook. 
...
![](figures/house-spenders.png){width="500%"}
%"}
...
fb_top_plot <- ggplot(fb_ads, aes(x = impressions_millions, 
